            ...
            <div className="p-5">
              ...
              <ul className="flex flex-col gap-3">
                ...
                {messageInfo.receiverId &&
                  !Array.isArray(messageInfo.readBy) && (
                    <li className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <img
                          src={
                            messageInfo.receiverId.profilePic ||
                            "/default-avatar.png"
                          }
                          alt={messageInfo.receiverId.name}
                          className="w-8 h-8 rounded-full object-cover border border-gray-300 dark:border-zinc-700"
                        />
                        <span className="text-gray-800 dark:text-gray-200 font-medium">
                          {messageInfo.receiverId.name}
                        </span>
                      </div>
                      <span className="text-xs text-gray-500 dark:text-gray-400">
                        {messageInfo.isRead ? "Seen" : "Sent"} at{" "}
                        {new Date(
                          messageInfo.isRead
                            ? messageInfo.seenAt
